const base_url = "https://todo-app-server-dhan.herokuapp.com/"
...
function getReadMyTodos() {
  ...
  .done((todos) => {
    $("#todoList").empty()
      ...
      let isDone = (el.status) ? 'selected' : '""'
      let isUndone = (!el.status) ? 'selected' : '""'
        ...
        <div class="card col">
          <div class="card-body">
            <div>
              ...
              <span class="badge badge-pill ${badge}">${statusBadge}</span>
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" id="inputModalStatus-${el.id}" required>
                    ...
                    <option value="true" ${isDone}>Done</option>
                    <option value="false" ${isUndone}>Undone</option>
                  </select>
                </div>
                <div class="form-group">
...
function addTodo() {
  ...
  .done((todo) => {
    ...
    let isDone = (todo.status) ? 'selected' : '""'
    let isUndone = (!todo.status) ? 'selected' : '""'
...
function update(id) {
  ...
  console.log(due_date, title, "<< due date");
  $.ajax({
...
function cancelAdd(e) { // button Cancel add todo form
  e.preventDefault()
  $("#addTodo").slideUp()
  $("#addTodo-form").trigger('reset')
}
